  ...
  return <div className="drought-soil-monitor">
    ...
    <div className="drought-soil-grid">
      <section id="drought-monitor" className="drought-panel" aria-labelledby="drought-title">
        ...
        {selectedMap&&!mapFailed?<figure className="drought-map"><img src={selectedMap.url} onError={()=>setMapFailed(true)} alt={`${mapLabels[lang][mapKey]} · ${selectedMap.period}`} loading="lazy"/><figcaption>{selectedMap.period} · Copernicus GDO{selectedMap.periodVerified!==true?(zh?" · 日期待核实":" · date unverified"):""}</figcaption></figure>:<p role="status">{zh?"全球图层暂时无法读取；上方缓存点值仍保留其原始资料日期。":"The global layer cannot be loaded; cached point classifications retain their source dates."}</p>}
        {mapKey==="impactRisk"?<p className="fs-muted">{zh?"农业影响风险图例：红色为高、橙色为中、黄色为低；未着色不等于安全。":"Agricultural impact risk: red = high, orange = medium, yellow = low; uncoloured does not mean safe."}</p>:<div className="drought-legend" aria-label={zh?"SPI 图例":"SPI legend"}><span className="dry-3">{zh?"极端偏干":"Extreme dry"}</span><span className="dry-2">{zh?"严重偏干":"Severe dry"}</span><span className="dry-1">{zh?"中度偏干":"Moderate dry"}</span><span className="normal">{zh?"接近常年":"Near normal"}</span><span className="wet">{zh?"偏湿":"Wet"}</span></div>}
