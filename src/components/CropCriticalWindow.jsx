...
import DroughtSoilMonitor from "./DroughtSoilMonitor";
...
import {loadDroughtMonitor} from "../services/droughtMonitor.js";
const copy={
  zh:{title:"全球作物关键窗口",intro:"现在的天气，对哪些作物阶段更重要？先用季节日历定位检查对象。以下全部是本站整理的近似生育期模板，不是今年实测进度，也不是实时天气风险排名。",
    month:"查看月份",crop:"作物",all:"全部作物",current:"回到本月",priorities:"本月应核对的关键生育期",none:"所选作物本月没有模板中的关键窗口。收获期仍可能受洪涝等影响，不代表没有风险。",estimated:"季节估计 · 置信度中",stage:"近似阶段",weather:"查看代表点天气",source:"日历参考来源",reviewed:"模板整理日期",sensitivity:"生育期敏感度（本站规则）",priorityNote:"按生育期敏感度排序，同分按固定编号排序；不是按国家重要性排序。已有代表点天气和干旱背景，但没有按产区面积加权的区域观测、产区份额或贸易权重，因此不计算全球农业天气风险或同步受灾分数。",
    calendar:"展开 12 个月季节日历",calendarHelp:"可横向滚动；点击月份格查看该月份，颜色之外还提供阶段缩写。各地播期、品种、灌溉和早晚季不同，这些代表性窗口不能覆盖全国所有农田。",region:"代表产区 / 作物",synchronization:"同步压力：尚无法判断",syncText:"进入敏感期 ≠ 正在受灾。代表点观测不能代替按种植面积加权的区域证据，因此不显示“0 个产区受灾”或正常绿色状态。",
...
export default function CropCriticalWindow({lang}) {
  ...
  const currentYear=new Date().getUTCFullYear(),[year,setYear]=useState(currentYear);
  useEffect(()=>{
  ...
  useEffect(()=>{
    const controller=new AbortController();
    loadDroughtMonitor(controller.signal).then(setDrought).catch(()=>{});
    return ()=>controller.abort();
  },[]);
  ...
  return <section id="crop-windows" className="section food-system">
    ...
    <div className="fs-controls"><label className="fs-control">{t.month}<select aria-label={t.month} value={month} onChange={e=>setMonth(+e.target.value)}>{Array.from({length:12},(_,i)=><option value={i+1} key={i}>{monthName(i+1)}</option>)}</select></label>
      ...
      <button type="button" onClick={()=>{setMonth(currentMonth);setYear(currentYear);}}>{t.current}</button></div>
    <h3>{lang==="zh"?"所选年月：季节关键窗口":"Selected period: seasonal critical windows"} · {year} · {monthName(month)}</h3><p className="fs-muted">{lang==="zh"?"按季节模板敏感度排序，不是按天气损伤或国家重要性排名。模板不随年份改变，不代表该年实测进度；下方天气按所选年月展示。":"Sorted by seasonal stage sensitivity, not weather damage or country importance. Templates do not vary by year or measure that year's progress; weather below follows the selected period."}</p>
    ...
    <div id="local-crop-weather" style={{scrollMarginTop:115}}><LocalCropWeather bundle={weather} rows={rows} month={month} year={year} lang={lang} selection={weatherSelection} setSelection={setWeatherSelection}/></div>
    <DroughtSoilMonitor drought={drought} weather={weather} rows={rows} month={month} year={year} lang={lang} selection={weatherSelection} setSelection={setWeatherSelection}/>
