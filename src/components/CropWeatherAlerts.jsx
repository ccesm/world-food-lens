...
import {ALERT_REVIEWED,productionSource} from "../data/cropWeatherAlerts.js";
import {alertSnapshot,productionContextShare} from "../services/cropWeatherAlerts.js";
...
export default function CropWeatherAlerts({lang}) {
  ...
  return <section id="crop-weather-alerts" className="crop-weather-alerts" aria-labelledby="weather-alert-title">
    ...
    <p>{zh?"每条报告单列受灾面积。部分卡片另给同作物的全国／欧盟产量权重，帮助判断规模；这不是受灾产量，也不能把重叠地区相加。":"Each report shows affected area separately. Some cards also show national/EU output share of the same crop for scale; this is not damaged production, and overlapping regions cannot be added."}</p>
    ...
    <div className="fs-factor-grid">{snapshot.rows.map(r=><article className={`fs-card weather-alert-${r.level}`} key={r.id}>
      ...
      <div className="weather-alert-scale"><p><b>{zh?"经核实受灾面积":"Verified affected cropland"}:</b> {zh?"官方报告未提供可对应此事件的公顷数":"No event-specific hectares published in these reports"}</p>
      <p><b>{zh?"全球同作物产量权重（背景）":"Share of world output, same crop (context)"}:</b> {r.productionContext?`${r.productionContext.crop[lang]} ${productionContextShare(r.productionContext)}% · ${r.productionContext.scope[lang]}`:(zh?"缺少与受灾区域及作物一致的可靠口径":"No reliable region-and-crop match")}</p>
      {r.productionContext&&<small>{zh?"USDA 2026/27 预测产量，千吨":"USDA 2026/27 forecast production, thousand metric tons"}: {r.productionContext.production.toLocaleString()} / {r.productionContext.worldProduction.toLocaleString()} · <a href={productionSource.url} target="_blank" rel="noreferrer">{productionSource.name} ↗</a></small>}</div>
