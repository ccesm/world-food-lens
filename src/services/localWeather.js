const DAY=86400000;
export function weatherSummary(record,calendar,selectedMonth,now=Date.now()){
  ...
  const stale=!Number.isFinite(fetched)||fetched>now+300000||now-fetched>3*DAY||lag>10||record.status!=="ok";
  let run=0,dry=0;
  ...
  return {start:days[0].date,end,stale,lag:Math.floor(lag),max:Math.max(...days.map(d=>d.max)),
    ...
    interpret:!stale&&selectedMonth===currentMonth,days};
}
